%NANSUM - overloaded nansum for SegArray
%
%   R = NANSUM(S)
%   R = NANSUM(S,DIM)
%
% Return value is a full row vector if DIM = 1, a SegArray column
% vector if DIM = 2, error if DIM > 2
...
% Note: currently maintained in parallel with sum.m
function R = nansum(S,dim)
    if ~exist('dim','var') || isempty('dim')
        dim = 1 + (size(S.bpts,1)==1);
    else
        try
      ...
      case 1 %% sum with the grain of SegArray
        if isempty(S)
            ...
            if M == 1
                ...
                R = full(nansum(sparse(I,J,wgtvals,M,N)));
            end
        end
